...
# Flag to control skipping tests
skip_all_tests = False

if os.getenv("TEST_TUPLE_VERIFICATION", "False").lower() == "false":
    skip_all_tests = True


# Use the flag to skip all tests
skip_test_trivial_verify = skip_all_tests
skip_neg_verify_pass = skip_all_tests
skip_neg_verify_fail = skip_all_tests
skip_and_verify_pass = skip_all_tests
skip_and_verify_fail = skip_all_tests
skip_or_verify_pass = skip_all_tests
skip_or_verify_pass = skip_all_tests
skip_paraphrase_verify_pass = skip_all_tests
skip_paraphrase_verify_fail = skip_all_tests
skip_conditional_verify_pass = skip_all_tests
skip_conditional_verify_fail = skip_all_tests
skip_consequence_quantity_verify_pass = skip_all_tests
skip_consequence_quantity_verify_fail = skip_all_tests
skip_consequence_time_verify_pass = skip_all_tests
skip_consequence_time_verify_fail = skip_all_tests
...
async def test_trivial_verify():
    ...
    assert (
        result_sync.valid == True
    ), "Verification should always pass for Trivial class, sync version"
    assert isinstance(
        result_sync, VerificationResult
    ), "Result should be an instance of VerificationResult, sync version"
...
async def test_neg_verify_pass():
    ...
    assert (
        result_sync.valid == True
    ), f"Verification failed. Reasoning: {result.reasoning}, sync version"
    assert (
        len(result_sync.reasoning) > 0
    ), "Verification reasoning should not be empty, sync version"
...
async def test_neg_verify_fail():
    ...
    assert (
        result_sync.valid == False
    ), "Verification should fail for incorrect negation, sync version"
    assert (
        len(result_sync.reasoning) > 0
    ), "Verification reasoning for failure should not be empty, sync version"
...
async def test_and_verify_fail():
    ...
    assert (
        result.valid == False
    ), "Verification should fail for incorrect AND combination"
    assert (
        len(result.reasoning) > 0
    ), "Verification reasoning for failure should not be empty"
    ...
    assert (
        result_sync.valid == False
    ), "Verification should fail for incorrect AND combination, sync version"
    assert (
        len(result_sync.reasoning) > 0
    ), "Verification reasoning for failure should not be empty, sync version"
...
async def test_or_verify_fail():
    ...
    assert (
        result.valid == False
    ), "Verification should fail for incorrect OR combination"
    assert (
        len(result.reasoning) > 0
    ), "Verification reasoning for failure should not be empty"
    ...
    assert (
        result_sync.valid == False
    ), "Verification should fail for incorrect OR combination, sync version"
    assert (
        len(result_sync.reasoning) > 0
    ), "Verification reasoning for failure should not be empty, sync version"
...
@pytest.mark.asyncio
@pytest.mark.skipif(
    skip_paraphrase_verify_pass, reason="Skipping paraphrase verify test"
)
async def test_paraphrase_verify_pass():
    ...
    sample_output = Paraphrase.OutputFormat(
        para_P=ForecastingQuestion(
            ...
            body="This question resolves YES if, before the start of 2030, SpaceX accomplishes a verified human touchdown on Mars. Verification must come from both SpaceX and independent space organizations. Resolves NO otherwise or if the deadline passes without such a landing.",
            resolution_date=datetime(2030, 1, 1),
            question_type="binary",
        )
    )
    ...
    assert result.valid == True, f"Verification failed. Reasoning: {result.reasoning}"
    assert len(result.reasoning) > 0, "Verification reasoning should not be empty"
    assert paraphrase_instance.verify_length(
        sample_output, base_sentences
    ...
    assert paraphrase_instance.verify_length(
        sample_output, base_sentences
    ), "Paraphrase length verification failed, sync version"


@pytest.mark.asyncio
@pytest.mark.skipif(
    skip_paraphrase_verify_fail, reason="Skipping paraphrase verify test"
)
async def test_paraphrase_verify_fail_omit_key_info():
    paraphrase_instance = Paraphrase()
    ...
    incorrect_output = Paraphrase.OutputFormat(
        para_P=ForecastingQuestion(
            id=uuid4(),
            title="Is a human Mars landing going to occur prior to 2030?",
            body="This question resolves YES if, before the start of 2030, a verified human touchdown on Mars is accomplished. The landing must be officially confirmed by SpaceX and independent space agencies. Resolves NO otherwise or if the deadline passes without such a landing.",
            resolution_date=datetime(2030, 1, 1),
            question_type="binary",
        )
    )
    ...
    assert (
        result.valid == False
    ), "Verification should fail for paraphrase omitting key information"
    assert (
        len(result.reasoning) > 0
    ), "Verification reasoning for failure should not be empty"
    assert paraphrase_instance.verify_length(
        incorrect_output, base_sentences
    ), "Paraphrase length verification failed"
    ...
    assert (
        result_sync.valid == False
    ), "Verification should fail for paraphrase omitting key information, sync version"
    assert (
        len(result_sync.reasoning) > 0
    ), "Verification reasoning for failure should not be empty, sync version"
    assert paraphrase_instance.verify_length(
...
async def test_paraphrase_verify_fail():
    ...
    assert (
        result_sync.valid == False
    ), "Verification should fail for incorrect paraphrase, sync version"
    assert (
        len(result_sync.reasoning) > 0
    ), "Verification reasoning for failure should not be empty, sync version"
    assert paraphrase_instance.verify_length(
...
@pytest.mark.asyncio
@pytest.mark.skipif(
    skip_conditional_verify_pass, reason="Skipping conditional verify test"
)
async def test_conditional_verify_pass():
    ...
    base_sentences = Conditional.BaseSentenceFormat(
        P=sample_question_P, Q=sample_question_Q
    )
    ...
    assert result.valid == True, f"Verification failed. Reasoning: {result.reasoning}"
    assert len(result.reasoning) > 0, "Verification reasoning should not be empty"
    assert conditional_instance.verify_length(
        sample_output, base_sentences
    ...
    assert conditional_instance.verify_length(
        sample_output, base_sentences
    ), "Conditional length verification failed, sync version"


@pytest.mark.asyncio
@pytest.mark.skipif(
    skip_conditional_verify_fail, reason="Skipping conditional verify test"
)
async def test_conditional_verify_fail_incorrect_direction():
    conditional_instance = Conditional()
    ...
    incorrect_output = Conditional.OutputFormat(
        Q_given_P=ForecastingQuestion(
            id=uuid4(),
            title="Given that US inflation rate drops below 2% by the end of 2024, will the US Federal Reserve raise interest rates in Q3 2024?",
            body="This question resolves YES if both of the following conditions are met:\n1. The US Consumer Price Index (CPI) shows an annual inflation rate below 2% for December 2024, as reported by the US Bureau of Labor Statistics, AND\n2. The US Federal Reserve increases the federal funds rate at any of its meetings in July, August, or September 2024.\nResolves NO if condition 1 is met but condition 2 is not.\nResolves N/A if the US inflation rate does not drop below 2% by the end of 2024.",
            resolution_date=datetime(2024, 9, 30),
            question_type="conditional_binary",
        )
    )
    ...
    assert (
        result.valid == False
    ), "Verification should fail for incorrect conditional direction"
    assert (
        len(result.reasoning) > 0
    ), "Verification reasoning for failure should not be empty"
    assert conditional_instance.verify_length(
    ...
    assert (
        result_sync.valid == False
    ), "Verification should fail for incorrect conditional direction, sync version"
    assert (
        len(result_sync.reasoning) > 0
    ), "Verification reasoning for failure should not be empty, sync version"
    assert conditional_instance.verify_length(
...
async def test_conditional_verify_complete_mess_fail():
    conditional_instance = Conditional()
    ...
    assert (
        result_sync.valid == False
    ), "Verification should fail for incorrect conditional, sync version"
    assert (
        len(result_sync.reasoning) > 0
    ), "Verification reasoning for failure should not be empty, sync version"
    assert conditional_instance.verify_length(
...
@pytest.mark.asyncio
@pytest.mark.skipif(
    skip_consequence_quantity_verify_pass,
    reason="Skipping consequence quantity verify test",
)
async def test_consequence_quantity_verify_pass():
    ...
    assert (
        result.valid == True
    ), f"Quantity verification failed. Reasoning: {result.reasoning}"
    assert len(result.reasoning) > 0, "Verification reasoning should not be empty"
    assert consequence_instance.verify_length(
        sample_output, base_sentences
    ...
    assert (
        result_sync.valid == True
    ), f"Quantity verification failed. Reasoning: {result.reasoning}, sync version"
    assert (
        len(result_sync.reasoning) > 0
    ), "Verification reasoning should not be empty, sync version"
    assert consequence_instance.verify_length(
        sample_output, base_sentences
    ), "Consequence length verification failed, sync version"


@pytest.mark.asyncio
@pytest.mark.skipif(
    skip_consequence_time_verify_pass, reason="Skipping consequence time verify test"
)
async def test_consequence_time_verify_pass():
    ...
    sample_question_P = ForecastingQuestion(
        id=uuid4(),
        title="Will quantum computers achieve quantum supremacy for a practical problem by the start of 2028?",
    ...
    sample_output = Consequence.OutputFormat(
        cons_P=ForecastingQuestion(
            id=uuid4(),
            title="Will quantum computers achieve quantum supremacy for a practical problem by the start of 2029?",
            body="Resolves YES if a quantum computer demonstrably solves a practical problem faster than any classical computer before January 1, 2029. The achievement must be published in a peer-reviewed scientific journal and acknowledged by the scientific community. Resolves NO otherwise.",
            resolution_date=datetime(2029, 1, 1),
            question_type="binary",
            metadata={"consequence_type": "time"},
        )
    )
    ...
    assert (
        result.valid == True
    ), f"Time verification failed. Reasoning: {result.reasoning}"
    assert len(result.reasoning) > 0, "Verification reasoning should not be empty"
    assert consequence_instance.verify_length(
        sample_output, base_sentences
    ), "Consequence length verification failed"
    ...
    assert (
        result_sync.valid == True
    ), f"Time verification failed. Reasoning: {result.reasoning}, sync version"
    assert (
        len(result_sync.reasoning) > 0
    ), "Verification reasoning should not be empty, sync version"
    assert consequence_instance.verify_length(
...
@pytest.mark.asyncio
@pytest.mark.skipif(
    skip_consequence_misc_verify_pass, reason="Skipping consequence misc verify test"
)
async def test_consequence_misc_verify_pass():
    ...
    assert (
        result.valid == True
    ), f"Misc verification failed. Reasoning: {result.reasoning}"
    assert len(result.reasoning) > 0, "Verification reasoning should not be empty"
    assert consequence_instance.verify_length(
        sample_output, base_sentences
    ), "Consequence length verification failed"
    ...
    assert (
        result_sync.valid == True
    ), f"Misc verification failed. Reasoning: {result.reasoning}, sync version"
    assert (
        len(result_sync.reasoning) > 0
    ), "Verification reasoning should not be empty, sync version"
    assert consequence_instance.verify_length(
...
async def test_consequence_misc_multiple_axes_verify_pass():
    consequence_instance = Consequence()
...
@pytest.mark.asyncio
@pytest.mark.skipif(
    skip_consequence_misc_verify_fail, reason="Skipping consequence misc verify test"
)
async def test_consequence_misc_verify_fail():
    ...
    assert (
        result.valid == False
    ), "Misc verification should fail for incorrect consequence"
    assert (
        len(result.reasoning) > 0
    ), "Verification reasoning for failure should not be empty"
    assert consequence_instance.verify_length(
    ...
    assert (
        result_sync.valid == False
    ), "Misc verification should fail for incorrect consequence, sync version"
    assert (
        len(result_sync.reasoning) > 0
    ), "Verification reasoning for failure should not be empty, sync version"
    assert consequence_instance.verify_length(
...
@pytest.mark.asyncio
@pytest.mark.skipif(
    skip_consequence_quantity_verify_fail,
    reason="Skipping consequence quantity verify test",
)
async def test_consequence_quantity_wrong_direction_verify_fail():
    consequence_instance = Consequence()
    ...
    assert (
        result.valid == False
    ), "Quantity verification should fail for incorrect consequence"
    assert (
        len(result.reasoning) > 0
    ), "Verification reasoning for failure should not be empty"
    assert consequence_instance.verify_length(
    ...
    assert (
        result_sync.valid == False
    ), "Quantity verification should fail for incorrect consequence, sync version"
    assert (
        len(result_sync.reasoning) > 0
    ), "Verification reasoning for failure should not be empty, sync version"
    assert consequence_instance.verify_length(
...
async def test_consequence_quantity_wrong_detail_verify_fail():
    consequence_instance = Consequence()
    ...
    incorrect_output = Consequence.OutputFormat(
        cons_P=ForecastingQuestion(
            id=uuid4(),
            title="Will the global tablet market exceed 1.3 billion units sold in 2026?",
            body="Resolves YES if the total number of tablets sold worldwide in the year 2026 surpasses 1.3 billion units, as reported by IDC, Gartner, or another reputable technology market research firm. Resolves NO otherwise.",
            resolution_date=datetime(2026, 12, 31),
            question_type="binary",
        )
    )
...
@pytest.mark.asyncio
@pytest.mark.skipif(
    skip_consequence_time_verify_fail, reason="Skipping consequence time verify test"
)
async def test_consequence_time_wrong_direction_verify_fail():
    consequence_instance = Consequence()
    ...
    assert (
        result.valid == False
    ), "Time verification should fail for incorrect consequence"
    assert (
        len(result.reasoning) > 0
    ), "Verification reasoning for failure should not be empty"
    assert consequence_instance.verify_length(
    ...
    assert (
        result_sync.valid == False
    ), "Time verification should fail for incorrect consequence, sync version"
    assert (
        len(result_sync.reasoning) > 0
    ), "Verification reasoning for failure should not be empty, sync version"
    assert consequence_instance.verify_length(
